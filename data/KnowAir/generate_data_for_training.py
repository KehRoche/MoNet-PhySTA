...
def generate_train_val_test(args):
    data = np.load('data/'+args.dataset + '/' + args.dataset + '.npy') # shape: (T, N, F)
    ...
    # 1. 准备训练集切片（只在 time 轴上切分，不含 label/掩码）
    t_train_end = idx_val[0] - args.seq_length_x
    x_train = data[:t_train_end, :, :F]  # (T_train, N, F)

    # 2. 展平为 (样本数, 特征数) 以符合 StandardScaler 要求
    x_train_flat = x_train.reshape(-1, F)  # (T_train * N, F)

    # 3. fit 标准化器
    scaler = StandardScaler()
    scaler.fit(x_train_flat)  # 要求输入为 2D：n_samples × n_features :contentReference[oaicite:0]{index=0}

    # 4. 对全量数据做 transform，并 reshape 回 (T, N, F)
    data_flat = data[..., :F].reshape(-1, F)  # (T * N, F)
    data_scaled_flat = scaler.transform(data_flat)  # 同样 2D 输入输出 :contentReference[oaicite:1]{index=1}
    data[..., :F] = data_scaled_flat.reshape(T, N, F)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='KnowAir', help='dataset name')
